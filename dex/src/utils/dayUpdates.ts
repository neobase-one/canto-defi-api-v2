...
export async function updateFactoryDayData(event: providers.Log) {
  ...
  const factory = await prisma.stableswapFactory.findFirstOrThrow({where: {id: FACTORY_ADDRESS}})
...
export async function updatePairDayData(event: providers.Log) {
  ...
  // load
  const pair = await prisma.pair.findFirstOrThrow({where: {id: ethers.utils.getAddress(ethers.utils.getAddress(event.address))}})

  const pairDayData = await prisma.pairDayData.upsert({
    where: {id: `${ethers.utils.getAddress(event.address)}-${new Decimal(dayId).toString()}`},
    create: {
      id: `${ethers.utils.getAddress(event.address)}-${new Decimal(dayId).toString()}`,
      date: dayStartTimestamp,
...
export async function updatePairHourData(event: providers.Log) {
  ...
  let hourPairId = `${ethers.utils.getAddress(event.address)}-${new Decimal(hourIndex).toString()}`;
  
  // load
  const pair = await prisma.pair.findFirstOrThrow({where: {id: ethers.utils.getAddress(event.address)}})
