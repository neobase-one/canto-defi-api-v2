...
export const factoryContract = new ethers.Contract(config.canto.contracts.baseV1Factory.addresses[0], BaseV1FactoryABI, provider);
...
export async function convertTokenToDecimal(
  ...
  return new Decimal(tokenAmount.toString()).div(await exponentToBigDecimal(exchangeDecimals));
}

export async function isCompleteMint(mintId: string): Promise<boolean> {
  return (await prisma.mint.findFirstOrThrow({where: { id: mintId }})).sender != null
}
...
export async function createLiquidityPosition(exchange: string, user: string) {
  if(!await prisma.liquidityPosition.count({where: {id: `${exchange}-${user}`}})){
  ...
  if (liquidityPosition === null) {
    console.log("ERROR: LiquidityTokenBalance is null", [`${exchange}-${user}`]);
  }
...
export async function createLiquiditySnapshot(
  ...
  const pair = await prisma.pair.findFirstOrThrow({where: {id: position.pairId}, include: {token0: true, token1: true}})
